...
export class MiningManager {
  constructor() {
    ...
    this.maxWorkers = navigator.hardwareConcurrency || 4 // CPU cores limit
    this.stats = {
      totalAttempts: 0,
      totalShares: 0,
      actualHashRate: 0,
  ...
  async createWorkers(hashRate) {
    ...
    const workerCount = Math.min(hashRate, this.maxWorkers)
    const promises = []
  ...
  handleWorkerMessage(message, workerId) {
    ...
    switch (type) {
      ...
      case "stats":
        ...
        // Calculate combined hash rate from all workers
        const totalActualRate = this.workers.reduce((sum, w) => {
          return sum + (w.lastHashRate || 0)
        }, 0)

        // Store this worker's rate
        const workerInfo = this.workers.find((w) => w.workerId === workerId)
        if (workerInfo) {
          workerInfo.lastHashRate = Number.parseFloat(data.actualHashRate)
        }
